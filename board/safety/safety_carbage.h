...
static bool carbage_tx_hook(const CANPacket_t *to_send) {
  ...
  if ((bus == 0) && (addr == 0x2E4)) {
    ...
    // FIXME: this sometimes blocks messages for no apparent reason I could find?
    // let's just not allow actuation with controls_allowed == false
